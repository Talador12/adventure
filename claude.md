...
Round 32 (in progress): Visual polish for Lobby + Game pages, continued UX improvements. Round 31 (complete): Fantasy animation system (10 keyframes, 15+ utility classes), hero gradient + shimmer, card hover effects, hono 4.12.5→4.12.7 security patch, GameContext decomposition (1896→1082 lines — types/data extracted to 4 new files), code splitting via React.lazy (703KB→split: main 389KB + Game 171KB + CharacterCreate 93KB + Lobby 26KB), prefers-reduced-motion accessibility, error handling overhaul (18 bare catch blocks replaced with logError(), 4 JWT/fallback catches kept silent). Round 30 (complete): Lobby password protection, save state indicator, auth gating, dashboard polish. Round 29 (complete): Spectator mode, campaign visibility, public campaign browser. Previous: Rounds 26-28 (D1, chat, OAuth, character wizard, battle map, animations).
