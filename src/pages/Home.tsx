                  ...
                  {campaigns.map((c) => (
                    <div key={c.roomId} className="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                      ...
                      <div className="min-w-0 flex-1 cursor-pointer" onClick={() => navigate(`/game/${c.roomId}`)}>
                        ...
                        <div className="text-xs text-slate-500 dark:text-slate-400">{new Date(c.createdAt).toLocaleDateString()}</div>
                      </div>
                      <button
                        onClick={() => navigate(`/lobby/${c.roomId}`)}
                      ...
                      <button
                        onClick={() => navigate(`/game/${c.roomId}`)}
                      ...
                      <button
                        onClick={() => handleDeleteCampaign(c.roomId, c.name)}
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {characters.map((c) => (
                    <div key={c.id} className="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                      {/* Portrait thumbnail */}
                      {c.portrait ? (
                        <img src={c.portrait} alt="" className="w-9 h-9 rounded-lg object-cover shrink-0" />
                      ) : (
                        <div className="w-9 h-9 rounded-lg bg-slate-300 dark:bg-slate-700 flex items-center justify-center text-sm font-bold text-slate-600 dark:text-slate-300 shrink-0">
